...
impl IconElement {
    ...
    fn read<R: Read>(mut reader: R) -> io::Result<IconElement> {
        ...
        try!(reader.read_exact(&mut ostype));
...
mod tests {
    ...
    fn read_icon_family_with_fake_elements() {
        ...
        assert_eq!(*b"quux", family.elements()[0].ostype());
        assert_eq!(6, family.elements()[0].data().len());
        assert_eq!(*b"baz!", family.elements()[1].ostype());
    ...
    fn write_icon_family_with_fake_elements() {
        let mut family = IconFamily::new();
        family.add_element(IconElement::new(*b"quux", b"foobar".to_vec()));
        family.add_element(IconElement::new(*b"baz!", b"#".to_vec()));
        let mut output: Vec<u8> = vec![];
        family.write(&mut output).expect("write failed");
